import { Gameboard } from './boards.js';
...
// Create 2 empty arrays for Player and Computer
let playerArr = [];
let compArr = [];

// Init Gameboard Factory Functions
const playerGameBoard = Gameboard(userBoard, playerArr, 'square');
const compGameBoard = Gameboard(compBoard, compArr, 'comp-square');

// Render Player and Computer board on UI
...
// User clicks and selects from the fleet
shipsDiv.forEach((el) => {
...
function paintBoard(target, length) {
  ...
  if (document.getElementsByClassName('hidden').length >= 4) {
    ...
    startGameBtn.style.background = 'lightgreen';
...
// Computer generates random ships after user clicks start game button
